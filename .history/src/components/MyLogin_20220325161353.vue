...
<!-- ----------------------下面是script文件----------------- -->
<script>
//  lang="ts"可以解决validate问题，但是还有其他问题
// console.log('项目开始运行')
export default {
  data() {
    console.log('data部分开始运行')
    return {
      // 这是登录表单的数据绑定对象
      LoginForm: {
        username: 'admin',
      ...
      LoginFormRules: {
        ...
        password: [
          ...
          { min: 6, max: 20, message: '密码应该是6到20个字符', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    // console.log('method部分开始运行')
    // 点击重置按钮，重置登录表单
    resetLoginForm () {
      this.$refs.LoginFormRef.resetFields()
    },
    Login () {
      console.log('Login部分开始运行')
      ...
      // 如果valid参数为true则验证通过
        ...
        // 1、将登录成功的token存到客户端的sessionStorage中
        // 1.1、项目中除了登录之外其他的API接口，必须在登录之后才能访问
        // 1.2 token只应在当前网站打开期间生效，所以将token存在sessionStorage中
        console.log(res)
      })
    }
  }
